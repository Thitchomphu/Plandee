...
export default function EventsScreen() {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={loading && items.length > 0} onRefresh={() => { void refresh(); }} tintColor={Theme.colors.primary} colors={[Theme.colors.primary]} />}>
    <View style={listStyles.heading}><View style={listStyles.headingCopy}><Text style={styles.title}>อีเวนต์ของฉัน</Text><Text style={listStyles.subtitle}>{items.length ? `${items.length} อีเวนต์ในแผนของคุณ` : 'วางแผนทุกช่วงเวลาสำคัญ'}</Text></View>{items.length ? <Pressable accessibilityRole="button" accessibilityLabel="สร้างอีเวนต์ใหม่" onPress={() => router.push('/event/new')} style={listStyles.create}><LinearGradient pointerEvents="none" colors={Theme.gradients.rose} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} /><FontAwesome6 name="plus" size={18} color="#FFFFFF" /></Pressable> : null}</View>
    ...
    {loadError ? <Pressable accessibilityRole="button" onPress={() => { void refresh(); }} style={styles.errorCard}><Text style={styles.errorTitle}>โหลดรายการอีเวนต์ไม่สำเร็จ</Text><Text style={styles.errorDetail}>แตะเพื่อลองใหม่ ตรวจสอบอินเทอร์เน็ตและการเข้าสู่ระบบ</Text></Pressable> : null}
    ...
    {!loading && !loadError && filtered.length === 0 ? <View style={cardStyles.emptyCard}><View style={cardStyles.emptyIcon}><FontAwesome6 name="calendar-days" size={24} color={Theme.colors.primary} /></View><Text style={cardStyles.emptyTitle}>{items.length ? 'ไม่มีอีเวนต์ในสถานะนี้' : 'ยังไม่มีอีเวนต์ของคุณ'}</Text><Text style={cardStyles.emptyBody}>{items.length ? 'ลองเลือกตัวกรองอื่นเพื่อดูอีเวนต์ทั้งหมด' : 'เริ่มสร้างอีเวนต์แรก แล้วรวมทุกอย่างไว้ในที่เดียว'}</Text>{!items.length ? <Pressable accessibilityRole="button" onPress={() => router.push('/event/new')} style={cardStyles.emptyAction}><Text style={cardStyles.emptyActionText}>สร้างอีเวนต์แรก</Text><FontAwesome6 name="arrow-right" size={12} color="#FFFFFF" /></Pressable> : null}</View> : null}
