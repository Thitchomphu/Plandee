...
export default function BudgetCategorySetupScreen() {
  ...
  const [amount, setAmount] = useState(existing?.planned ? String(existing.planned) : '');
  const [note, setNote] = useState(existing?.note ?? '');
  const save = () => {
    const planned = Number(amount.replace(/[^0-9]/g, ''));
