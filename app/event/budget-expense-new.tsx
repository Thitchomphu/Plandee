...
export default function BudgetExpenseFormScreen() {
  ...
  const [name, setName] = useState(expense?.name ?? ''); const [amount, setAmount] = useState(expense?.amount ? expense.amount.toLocaleString('en-US') : ''); const [paid, setPaid] = useState(expense?.paid ?? false);
  const save = () => { const parsed = currencyValue(amount); if (!name.trim() || !parsed || parsed <= 0) return Alert.alert('ข้อมูลไม่ถูกต้อง', 'กรุณากรอกชื่อรายการและจำนวนเงินเป็นตัวเลขบวก'); if (expenseId) updateBudgetExpense(expenseId, { name: name.trim(), amount: parsed, paid }); else addBudgetExpense({ eventId, categoryId, name: name.trim(), amount: parsed, paid }); router.back(); };
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><View style={styles.notch}><Text>—</Text></View><Text style={styles.title}>{expense ? 'แก้ไขค่าใช้จ่าย' : 'เพิ่มค่าใช้จ่าย'}</Text><Text style={styles.subtitle}>บันทึกรายการย่อยของหมวดงบประมาณ</Text><AppInput label="ชื่อรายการ" value={name} onChangeText={setName} placeholder="เช่น ค่าเช่าอุปกรณ์" /><CurrencyInput label="จำนวนเงิน (บาท)" value={amount} onChangeText={setAmount} placeholder="เช่น 15,000" /><AppButton title={paid ? 'สถานะ: จ่ายแล้ว' : 'สถานะ: ยังไม่จ่าย'} onPress={() => setPaid((value) => !value)} variant="outline" /><AppButton title={expense ? 'บันทึกการแก้ไข' : 'เพิ่มค่าใช้จ่าย'} onPress={save} /><AppButton title="ยกเลิก" onPress={() => router.back()} variant="outline" /></ScrollView></SafeAreaView>;
}
