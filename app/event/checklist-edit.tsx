...
export default function EditChecklistItemScreen() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    const { error } = await updateChecklistItem(id, { title: title.trim(), category, dueDate, budget: currencyValue(budget), note: note.trim(), done });
    setSaving(false);
