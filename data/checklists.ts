...
const responsibleError = (error: { message: string } | null) => error && /(assignee_name|responsible)/i.test(error.message)
...
export const hydrateChecklistItems = async () => {
  ...
  let usesLegacyAssignee = false;
  let { data, error } = await supabase.from('checklist_items').select(`${columns},responsible`);
  if (error && /responsible/i.test(error.message)) {
    usesLegacyAssignee = true;
    ({ data, error } = await supabase.from('checklist_items').select(`${columns},assignee_name`));
  }
  if (error) return false;
  checklistStore = ((data ?? []) as unknown as { id: string; event_id: string; title: string; category: ChecklistCategory; due_date: string | null; budget: number; note: string | null; done: boolean; responsible?: string | null; assignee_name?: string | null }[]).map((item) => ({ id: item.id, eventId: item.event_id, title: item.title, category: item.category, dueDate: item.due_date ?? '', budget: Number(item.budget), note: item.note ?? '', done: item.done, assigneeName: (usesLegacyAssignee ? item.assignee_name : item.responsible) ?? '' }));
...
export const addChecklistItem = async (item: Omit<ChecklistItem, 'id'>) => {
  ...
  let { data, error } = await supabase.from('checklist_items').insert({ event_id: item.eventId, title: item.title, category: item.category, due_date: item.dueDate || null, budget: item.budget, note: item.note, done: item.done, ...(item.assigneeName.trim() ? { responsible: item.assigneeName.trim() } : {}) }).select('id').single();
  if (error && /responsible/i.test(error.message)) {
    ({ data, error } = await supabase.from('checklist_items').insert({ event_id: item.eventId, title: item.title, category: item.category, due_date: item.dueDate || null, budget: item.budget, note: item.note, done: item.done, ...(item.assigneeName.trim() ? { assignee_name: item.assigneeName.trim() } : {}) }).select('id').single());
  }
  if (error) return { item: null, error: responsibleError(error) };
...
export const updateChecklistItem = async (id: string, patch: Partial<ChecklistItem>) => {
  ...
  let { error } = await supabase.from('checklist_items').update({ title: next.title, category: next.category, due_date: next.dueDate || null, budget: next.budget, note: next.note, done: next.done, ...(next.assigneeName !== current.assigneeName ? { responsible: next.assigneeName.trim() } : {}) }).eq('id', id);
  if (error && /responsible/i.test(error.message)) {
    ({ error } = await supabase.from('checklist_items').update({ title: next.title, category: next.category, due_date: next.dueDate || null, budget: next.budget, note: next.note, done: next.done, ...(next.assigneeName !== current.assigneeName ? { assignee_name: next.assigneeName.trim() } : {}) }).eq('id', id));
  }
  if (!error) {
    checklistStore = checklistStore.map((item) => item.id === id ? next : item);
    await refreshTaskReminders(checklistStore);
  }
  return { error: responsibleError(error) };
};
