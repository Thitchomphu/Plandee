...
begin
  ...
  values (auth.uid(), p_title, p_kind, coalesce(nullif(btrim(p_event_date), '')::date, current_date), p_event_date, p_venue, greatest(coalesce(p_budget, 0), 0))
  ...
  for item in select value from jsonb_array_elements(coalesce(p_checklist, '[]'::jsonb)) loop
    ...
    values (created_event.id, item ->> 'title', category_name, nullif(btrim(item ->> 'dueDate'), '')::date, coalesce((item ->> 'budget')::numeric, 0), item ->> 'note', false)
