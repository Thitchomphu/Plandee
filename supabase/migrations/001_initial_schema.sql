...
begin
  ...
  values (auth.uid(), p_title, p_kind, case when p_event_date ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then p_event_date::date else current_date end, p_event_date, p_venue, greatest(coalesce(p_budget, 0), 0))
  ...
  for item in select value from jsonb_array_elements(coalesce(p_checklist, '[]'::jsonb)) loop
    ...
    values (created_event.id, item ->> 'title', category_name, case when item ->> 'dueDate' ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then (item ->> 'dueDate')::date else null end, coalesce((item ->> 'budget')::numeric, 0), item ->> 'note', false)
